...
function weierstrass(curveDef) {
  ...
  const slcNum = (b, from16, to) => bytesToNumberBE(b.slice(from16, to));
  class Signature {
...
// apps/backend/scripts/grid-hsk.ts
...
var sha2564 = /* @__PURE__ */ createHasher3(() => new SHA2562());
...
// packages/shared/dist/eip712.js
var EIP712_DOMAIN_NAME = "MingaConservationAgreement";
var EIP712_DOMAIN_VERSION = "1";
var milestoneApprovalTypes = {
  MilestoneApproval: [
...
function buildApprovalTypedData(chainId, agreement, message) {
  return {
...
// packages/shared/dist/abi.js
var conservationAgreementAbi = [
  {
    "inputs": [
      {
        "components": [
...
var agreementFactoryRegistryAbi = [
  {
    "anonymous": false,
    "inputs": [
      {
...
// packages/shared/dist/grid.js
var METER_DOMAIN_NAME = "MingaGridMeter";
var METER_DOMAIN_VERSION = "1";
var meterReadingTypes = {
  MeterReading: [
...
function getMeterDomain(chainId) {
  return {
...
function buildMeterReadingTypedData(chainId, reading) {
  return {
...
async function verifyMeterBatch(chainId, batch, expectedDevice) {
  const problems = [];
  let totalWh = 0;
  let previousNonce = -1;
  ...
  for (let i = 0; i < batch.readings.length; i++) {
    ...
    try {
      ...
      problems.push({ index: i, code: "BAD_SIGNATURE", detail: error.message });
      continue;
    }
...
var BASELINE_METHOD = "mean-of-baseline-days/v1";
function buildBaselineProfile(readings, intervalSeconds, utcOffsetSeconds) {
...
function baselineWhForWindow(profile, windowStart, windowEnd) {
  let total = 0;
...
function settlementRecord(result) {
  return {
...
function hashSettlement(result) {
  return canonicalize(settlementRecord(result));
}
function hashMeterBatch(batch) {
  return canonicalize({
...
// packages/shared/dist/grid-keys.js
var DEVICE_SEED = "minga-grid/dev/device/1";
var AGENT_SEED = "minga-grid/dev/agent/1";
var SITE_PAYEE_SEED = "minga-grid/dev/site-payee/1";
var TREASURY_SEED = "minga-grid/dev/treasury/1";
function devKey(seed) {
  return keccak256(toBytes(seed));
}
function fromEnvOrSeed(envValue, seed) {
  const raw = envValue?.trim();
  if (raw && /^0x[0-9a-fA-F]{64}$/.test(raw))
    return privateKeyToAccount(raw);
  return privateKeyToAccount(devKey(seed));
}
function deviceAccount(env = process.env) {
  return fromEnvOrSeed(env.DEVICE_PRIVATE_KEY, DEVICE_SEED);
}
function agentAccount(env = process.env) {
  return fromEnvOrSeed(env.AGENT_PRIVATE_KEY, AGENT_SEED);
}
function sitePayeeAccount(env = process.env) {
  return fromEnvOrSeed(env.SITE_PAYEE_PRIVATE_KEY, SITE_PAYEE_SEED);
}
function treasuryAccount(env = process.env) {
  return fromEnvOrSeed(env.TREASURY_PRIVATE_KEY, TREASURY_SEED);
}

// packages/shared/dist/meter-sim.js
var METER_CHAIN_ID = 133;
var SITE_NAME = "Cold-storage warehouse, Valle del Cauca (site/co-valle-001)";
var INTERVAL_SECONDS = 900;
var UTC_OFFSET_SECONDS = -5 * 3600;
var FIRST_BASELINE_DAY_UTC = Date.UTC(2026, 8, 14, 22, 0, 0) / 1e3;
var EVENT_DAY_UTC = Date.UTC(2026, 8, 19, 22, 0, 0) / 1e3;
var BASELINE_DAYS = 5;
var DAY = 86400;
var PROFILE_HOURS = 5;
var INTERVALS_PER_DAY = PROFILE_HOURS * 3600 / INTERVAL_SECONDS;
var EVENT_WINDOW_OFFSET = 3600;
var EVENT_WINDOW_SECONDS = 3 * 3600;
var ORDINARY_WH = [6e4, 1e5, 9e4, 7e4, 5e4];
var DELIVERED_WH = [6e4, 4e4, 36e3, 3e4, 5e4];
var SHORTFALL_WH = [6e4, 84e3, 76e3, 6e4, 5e4];
...
function profileFor(hourlyWh, rand) {
  const out = [];
...
async function signDay(account, siteId, dayStart, values, startingNonce) {
  const signed = [];
...
function batchOf(siteId, device, readings) {
  const first = readings[0];
  const last = readings[readings.length - 1];
  if (!first || !last)
    throw new Error("a batch needs at least one reading");
  return {
    siteId,
    device,
...
function sliceWindow(batch, windowStart, windowEnd) {
  const readings = batch.readings.filter((r) => r.reading.periodStart >= windowStart && r.reading.periodEnd <= windowEnd);
...
async function buildFixtures() {
  ...
  const delivered = batchOf(siteId, account.address, await signDay(account, siteId, EVENT_DAY_UTC, profileFor(DELIVERED_WH, mulberry32(777)), eventNonce));
  const shortfall = batchOf(siteId, account.address, await signDay(account, siteId, EVENT_DAY_UTC, profileFor(SHORTFALL_WH, mulberry32(778)), eventNonce));
...
// packages/shared/dist/settlement.js
var CRITICAL_PEAK_RATIO = 1.8;
var ELEVATED_PEAK_RATIO = 1.4;
var CRITICAL_RESERVOIR_PCT = 35;
function assessGrid(signal) {
  ...
  const stress = critical ? "critical" : elevated ? "elevated" : "normal";
  const price = (signal.spotPriceMicroUsdPerKwh / 1e6).toFixed(3);
  ...
  return {
    ...
    reason: stress === "normal" ? `the window costs USD ${price}/kWh, only ${ratio}x the day average, and ${reservoir}: no event is worth calling` : `the window costs USD ${price}/kWh, ${ratio}x the day average, ${reservoir}: ${stress} stress, dispatching`
  };
}
function step(steps, s) {
  steps.push(s);
  return s;
}
async function runSettlement(deps) {
  ...
  let research = null;
  if (deps.research) {
    const objective = `Corroborate reported grid stress on ${new Date(deps.signal.observedAt * 1e3).toISOString().slice(0, 10)} from public sources`;
    try {
      ...
      step(steps, {
        step: "corroborate",
        detail: `external research unavailable (${error.message}); continuing on the primary signal`,
        ok: false
      });
    }
  }
  ...
  step(steps, {
    step: "verify",
    detail: verification.ok ? `${verification.count} readings verified against device ${program.device}` : `${verification.problems.length} of ${verification.count} readings rejected`,
    ok: verification.ok,
  ...
  step(steps, {
    step: "baseline",
    detail: `${(baselineWh / 1e3).toFixed(1)} kWh expected from ${profile.days} ordinary days (${profile.method})`,
    ok: true,
  ...
  step(steps, {
    step: "measure",
    detail: `${(settlement.actualWh / 1e3).toFixed(1)} kWh measured, ${(settlement.avoidedWh / 1e3).toFixed(1)} kWh avoided`,
    ok: true,
    data: { actualWh: settlement.actualWh, avoidedWh: settlement.avoidedWh }
  });
  const met = settlement.avoidedWh >= program.committedWh;
  const committedKwh = (program.committedWh / 1e3).toFixed(1);
  const avoidedKwh = (settlement.avoidedWh / 1e3).toFixed(1);
  step(steps, {
    step: "decide",
    detail: met ? `commitment of ${committedKwh} kWh met (${avoidedKwh} kWh)` : `commitment of ${committedKwh} kWh missed (${avoidedKwh} kWh): refusing to settle`,
    ok: met,
...
// packages/shared/dist/grid-feed.js
var XM_BASE = "https://servapibi.xm.com.co";
var TRM_URL = "https://www.datos.gov.co/resource/32sa-8pi3.json?$limit=1&$order=vigenciadesde%20DESC";
var WINDOW_HOURS = [19, 20, 21];
var LOOKBACK_DAYS = 8;
var TIMEOUT_MS = 15e3;
...
async function fetchTrm() {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(`TRM responded ${res.status}`);
    const rows = await res.json();
    const value = Number(rows[0]?.valor);
    if (!Number.isFinite(value) || value <= 0)
      throw new Error("TRM payload had no usable rate");
    return value;
  } finally {
    clearTimeout(timer);
  }
}
function fallbackSignal(reason = "sources unreachable") {
  return {
    source: `fixture (${reason}) \u2014 representative Colombian evening peak, NOT live data`,
    observedAt: Math.floor(Date.now() / 1e3),
    observedDate: iso(/* @__PURE__ */ new Date()),
    spotPriceMicroUsdPerKwh: 313e3,
    dailyAverageMicroUsdPerKwh: 188e3,
    peakRatio: 1.66,
    reservoirPct: null
  };
}
...
async function fetchGridSignal() {
  ...
  try {
    ...
    if (!latest)
      throw new Error("XM returned no published days in the window");
    ...
    if (hourly.length === 0)
      throw new Error("XM day had no hourly values");
    const windowPrices = WINDOW_HOURS.map((h) => Number(values[`Hour${String(h).padStart(2, "0")}`])).filter((v) => Number.isFinite(v));
    ...
    let reservoirPct = null;
    try {
      const r = await postJson("/daily", {
      ...
      if (Number.isFinite(fraction))
        reservoirPct = fraction * 100;
    } catch {
      reservoirPct = null;
    }
    const toMicroUsd = (cop) => Math.round(cop / trm * 1e6);
    const signal = {
    ...
    return fallbackSignal(error.message);
  }
}

// apps/backend/scripts/grid-hsk.ts
var MILESTONE_AMOUNT = 75000000n;
var WINDOWS = 2;
var COMMITTED_WH = 5e5;
var TARIFF_MICRO_USD_PER_KWH = 15e4;
var COMMUNITY_BPS = 9000n;
var MIN_MACHINE_GAS = 2000000000000000n;
var TOP_UP = 5000000000000000n;
var EXPLORER = "https://testnet-explorer.hskchain.net";
var hskTestnet = defineChain({
  id: 133,
...
var STATE_FILE = resolve(ROOT, "grid-hsk-run.json");
var FRESH = process.argv.includes("--fresh");
...
var mUSD = (v) => `${(Number(v) / 1e6).toFixed(2)} mUSD`;
...
async function main() {
  ...
  const token = process.env.MOCK_USD_ADDRESS;
  ...
  const publicClient = createPublicClient({ chain: hskTestnet, transport: http(void 0, { timeout: 6e4 }) });
  const wallet = (a) => createWalletClient({ account: a, chain: hskTestnet, transport: http(void 0, { timeout: 6e4 }) });
  const send = async (a, label, params) => {
    const hash3 = await wallet(a).writeContract(params);
    const receipt2 = await publicClient.waitForTransactionReceipt({ hash: hash3, timeout: 12e4 });
    if (receipt2.status !== "success") throw new Error(`${label} reverted (${hash3})`);
    log(`  ${label}`, `${EXPLORER}/tx/${hash3}`);
    return receipt2;
  };
  section("Identities");
  ...
  for (const [name, a] of [["offtaker", offtaker], ["device", device], ["agent", agent]]) {
    log(`  ${name} HSK`, formatEther2(await publicClient.getBalance({ address: a.address })));
  }
  if (DRY_RUN) {
  ...
  for (const [name, a] of [["device", device], ["agent", agent]]) {
    const balance = await publicClient.getBalance({ address: a.address });
    if (balance >= MIN_MACHINE_GAS) {
      log(`  ${name}`, `already funded (${formatEther2(balance)} HSK)`);
      continue;
    }
    const hash3 = await wallet(offtaker).sendTransaction({ to: a.address, value: TOP_UP });
    await publicClient.waitForTransactionReceipt({ hash: hash3, timeout: 12e4 });
    log(`  ${name}`, `topped up with ${formatEther2(TOP_UP)} HSK \u2014 ${EXPLORER}/tx/${hash3}`);
  }
  let agreement;
  ...
  if (previous?.agreement) {
    ...
    const block2 = await publicClient.getBlock({ blockTag: "latest" });
    ...
    const input = {
      ...
      executionDeadline: now + 172800n,
      demoMode: true
    };
    const receipt2 = await send(offtaker, "createAgreement", {
      address: factory,
      abi: agreementFactoryRegistryAbi,
      functionName: "createAgreement",
      args: [input]
    });
    const ev = parseEventLogs({ abi: agreementFactoryRegistryAbi, logs: receipt2.logs, eventName: "AgreementCreated" })[0];
    agreement = ev.args.agreementAddress;
    log("  program", `${EXPLORER}/address/${agreement}`);
    saveState({ agreement, factory, token, createdAt: (/* @__PURE__ */ new Date()).toISOString() });
  }
  await waitForCode(publicClient, agreement);
  const read = (fn, args = []) => retry(
    `read ${fn}`,
    () => publicClient.readContract({ address: agreement, abi: conservationAgreementAbi, functionName: fn, args })
  );
  const termsHash = await read("termsHash");
  section("Both machines accept the terms");
  for (const [name, a, flag] of [["device", device, "communityAccepted"], ["agent", agent, "verifierAccepted"]]) {
    if (await read(flag)) {
      log(`  ${name}`, "already accepted");
      continue;
    }
  ...
  if (await read("funded")) {
    ...
    const tokenRead = (fn, args) => retry(
      `read ${fn}`,
      () => publicClient.readContract({ address: token, abi: mockUsdAbi, functionName: fn, args })
    );
  ...
  const outcome = await runSettlement({
    ...
    chain: {
      ...
      nonce: await read("milestoneNonce", [BigInt(milestoneId)]),
      demoMode: await read("demoMode"),
      fundedAt: await read("fundedAt"),
      executionDeadline: await read("executionDeadline"),
      now: block.timestamp
    },
...
The agent refused to settle: ${outcome.reason}`);
  ...
  const expectedSite = MILESTONE_AMOUNT * COMMUNITY_BPS / 10000n;
  ...
  saveState({
    ...
    factory,
    token,
    offtaker: offtaker.address,
    device: device.address,
    agent: agent.address,
    sitePayee: site.address,
    treasury: treasury.address,
    committedWh: COMMITTED_WH,
    milestoneAmount: MILESTONE_AMOUNT.toString(),
    communityBps: Number(COMMUNITY_BPS),
    lastSettledMilestone: milestoneId,
    avoidedWh: outcome.settlement.avoidedWh,
    baselineWh: outcome.settlement.baselineWh,
    actualWh: outcome.settlement.actualWh,
    evidenceHash: outcome.evidence.hash,
    releaseTx: receipt.transactionHash,
    explorer: `${EXPLORER}/address/${agreement}`,
...
async function balances(client, token, who) {
  const out = [];
...
async function waitForCode(client, address, timeoutMs = 9e4) {
  const deadline = Date.now() + timeoutMs;
  let announced = false;
  for (; ; ) {
    const code = await client.getCode({ address }).catch(() => void 0);
    if (code && code !== "0x") return;
    if (Date.now() > deadline) throw new Error(`No bytecode at ${address} after ${timeoutMs / 1e3}s`);
    if (!announced) {
...
async function waitUntil(label, check, timeoutMs = 9e4) {
  const deadline = Date.now() + timeoutMs;
  let announced = false;
  for (; ; ) {
