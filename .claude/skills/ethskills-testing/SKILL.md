...
> **Local overrides for this skill:** the brief mandates Hardhat as the contracts toolchain and the existing suite lives in `packages/contracts/test/` (56 Hardhat tests). Foundry (forge/cast/anvil 1.8.3) is installed only as a complement for fuzz and invariant tests and for `cast`; do not migrate or duplicate the Hardhat suite. Fork-testing sections do not apply: MINGA integrates no external on-chain protocols.
