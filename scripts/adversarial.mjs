...
var AGREEMENT = process.env.NEXT_PUBLIC_GRID_AGREEMENT_ADDRESS ?? "0x315DE6Ff84680012cf81bFd9C256032996809cEC";
var hsk = defineChain({
  id: 133,
...
var device = deviceAccount();
var agent = agentAccount();
async function main() {
  ...
  const publicClient = createPublicClient({ chain: hsk, transport: http(RPC, { timeout: 45e3 }), cacheTime: 0 });
  const wallet = createWalletClient({ account: relayer, chain: hsk, transport: http(RPC, { timeout: 6e4 }) });
  const read = (fn, args = []) => publicClient.readContract({ address: AGREEMENT, abi: conservationAgreementAbi, functionName: fn, args });
  console.log(`
Adversarial checks against ${AGREEMENT}`);
  console.log(`${EXPLORER}/address/${AGREEMENT}
`);
  const paid0 = await read("milestonePaid", [0n]);
  const next = Number(await read("nextMilestoneId"));
  console.log(`window 1 settled: ${paid0}   \xB7   next unsettled window: ${next + 1} of 2
`);
  const termsHash = await read("termsHash");
  const demoMode = await read("demoMode");
  const block = await publicClient.getBlock({ blockTag: "latest" });
  const sign2 = async (a, m) => {
  ...
  async function attack(name, expectation, build) {
...
`);
    ...
    try {
      ...
      console.log(`   could not even build the attempt: ${error.message.split("\n")[0]}`);
      return;
    }
    try {
      ...
      if (receipt.status === "success") {
        console.log(`   *** THE ATTACK SUCCEEDED. THIS IS A BUG. *** ${EXPLORER}/tx/${hash3}`);
      } else {
        console.log(`   rejected on chain, gas ${receipt.gasUsed}`);
        console.log(`   ${EXPLORER}/tx/${hash3}`);
      }
    } catch (error) {
      const message = error.message;
      const reason = /reverted with the following reason:\s*\n?(.+)/.exec(message)?.[1] ?? /execution reverted:?\s*(.*)/.exec(message)?.[1] ?? message.split("\n").find((l) => l.trim().length > 0) ?? "rejected";
      console.log(`   rejected before it could be mined: ${reason.trim()}`);
    }
  }
  const goodApproval = async (milestoneId) => ({
    milestoneId: BigInt(milestoneId),
    termsHash,
    evidenceHash: "0x" + "ab".repeat(32),
    amount: await read("milestoneAmounts", [BigInt(milestoneId)]),
  ...
  await attack(
    ...
    async () => {
      ...
      return [approval, await sign2(device, approval), await sign2(agent, approval)];
    }
  );
  await attack(
    ...
    async () => {
      ...
      const agentSig = await sign2(agent, approval);
  ...
  await attack(
    ...
    async () => {
      const approval = await goodApproval(next);
      const greedy = { ...approval, amount: approval.amount * 2n };
      return [greedy, await sign2(device, greedy), await sign2(agent, greedy)];
    }
  );
  await attack(
    ...
    async () => {
      const impostor = privateKeyToAccount("0x" + "77".repeat(32));
      const approval = await goodApproval(next);
      return [approval, await sign2(impostor, approval), await sign2(agent, approval)];
    }
  );
  console.log("\n" + "\u2500".repeat(72));
  const stillPaid = await read("milestonePaid", [0n]);
