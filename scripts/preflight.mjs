...
async function simulateCalls(client, parameters) {
  ...
  const results2 = block_calls.slice(0, -1);
...
async function verifyErc8010(client, parameters) {
  ...
  const results2 = await getAction(client, readContract, "readContract")({
  ...
  const data = results2[results2.length - 1]?.returnData;
...
function serialize(value, at) {
  ...
  switch (typeof value) {
    ...
    case "object": {
      ...
      return `{${keys.map((k) => `${JSON.stringify(k)}:${serialize(record2[k], `${at}.${k}`)}`).join(",")}}`;
    }
    default:
...
var APP = process.env.PREFLIGHT_APP_URL ?? "https://minga-grid.vercel.app";
var RPC = process.env.HSK_RPC_URL ?? "https://testnet.hsk.xyz";
var AGREEMENT = process.env.NEXT_PUBLIC_GRID_AGREEMENT_ADDRESS ?? process.env.GRID_AGREEMENT_ADDRESS ?? "0x315DE6Ff84680012cf81bFd9C256032996809cEC";
var TOKEN = process.env.MOCK_USD_ADDRESS ?? "0xEE2CDdBAaDB687E4932cB26295ADC5F5Ab3631E1";
var COMMITTED_WH = 5e5;
var TARIFF = 15e4;
var MIN_GAS = 1000000000000000n;
var hsk = defineChain({
  id: 133,
...
async function check(name, fn, soft = false) {
  try {
    ...
    record(soft ? "warn" : "fail", name, error.message.split("\n")[0] ?? "failed");
  }
}
async function main() {
  ...
  console.log(`
MINGA Grid pre-flight \xB7 ${(/* @__PURE__ */ new Date()).toISOString()}
`);
  const client = createPublicClient({ chain: hsk, transport: http(RPC, { timeout: 3e4 }), cacheTime: 0 });
  const read = (fn, args = []) => client.readContract({ address: AGREEMENT, abi: conservationAgreementAbi, functionName: fn, args });
  ...
  for (const [label, account] of [["meter", deviceAccount()], ["agent", agentAccount()]]) {
    await check(`${label} account`, async () => {
      const balance = await client.getBalance({ address: account.address });
      const gas = balance >= MIN_GAS ? "funded" : "LOW ON GAS";
      if (balance < MIN_GAS) throw new Error(`${account.address} has ${formatEther2(balance)} HSK`);
  ...
  await check("XM + exchange rate", async () => {
    ...
    return `${signal.observedDate} \xB7 USD ${(signal.spotPriceMicroUsdPerKwh / 1e6).toFixed(3)}/kWh \xB7 ${signal.peakRatio.toFixed(2)}x \xB7 ${verdict.stress}`;
  }, true);
  ...
  for (const scenario of ["delivered", "shortfall"]) {
    await check(`agent \xB7 ${scenario}`, async () => {
      ...
      const outcome = await runSettlement({
        ...
        chain: {
          termsHash: await read("termsHash"),
          milestoneId: nextWindow,
          milestoneAmount: await read("milestoneAmounts", [BigInt(nextWindow)]),
          nonce: await read("milestoneNonce", [BigInt(nextWindow)]),
          demoMode: await read("demoMode"),
          fundedAt: await read("fundedAt"),
          executionDeadline: await read("executionDeadline"),
          now: block.timestamp
        },
        signDevice: sign2(deviceAccount()),
        signAgent: sign2(agentAccount())
      });
      ...
      return expected ? `signs \xB7 ${(outcome.settlement.avoidedWh / 1e3).toFixed(1)} kWh avoided` : `refuses \xB7 ${outcome.reason}`;
    });
  }
  await check("execution deadline", async () => {
    const deadline = Number(await read("executionDeadline"));
    const left = deadline - Math.floor(Date.now() / 1e3);
  ...
  for (const path of ["/", "/app", "/api/grid/signal", "/api/grid/program"]) {
    await check(`app ${path}`, async () => {
      ...
      const res = await fetch(`${APP}${path}`, { signal: AbortSignal.timeout(3e4) });
  ...
  if (failed.length === 0 && warned.length === 0) {
    ...
    for (const w of warned) console.log(`  \xB7 ${w.name}: ${w.detail}`);
    if (!signalIsLive) console.log("  \xB7 Say on stage that the fixture is labelling itself. That is the behaviour, not a bug.");
  } else {
    console.log(`${failed.length} check(s) FAILED. Do not go on stage before reading these:`);
    for (const f of failed) console.log(`  \xB7 ${f.name}: ${f.detail}`);
    process.exitCode = 1;
  }
  console.log("");
}
main().catch((error) => {
  console.error(`
pre-flight crashed: ${error.message}`);
  process.exitCode = 1;
});
